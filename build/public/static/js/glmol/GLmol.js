/*
 GLmol - Molecular Viewer on WebGL/Javascript (0.47)
  ...
  This program uses
      THREEGLMOL.js 
         https://github.com/mrdoob/THREEGLMOL.js
         Copyright (c) 2010-2012 THREEGLMOL.js Authors. All rights reserved.
      jQuery
         http://jquery.org/
...
THREEGLMOL.ShaderLib.lambert.fragmentShader = THREEGLMOL.ShaderLib.lambert.fragmentShader.replace("gl_FrontFacing", "true");
THREEGLMOL.ShaderLib.lambert.vertexShader = THREEGLMOL.ShaderLib.lambert.vertexShader.replace(/\}$/, "#ifdef DOUBLE_SIDED\n if (transformedNormal.z < 0.0) vLightFront = vLightBack;\n #endif\n }");

var TV3 = THREEGLMOL.Vector3, TF3 = THREEGLMOL.Face3, TCo = THREEGLMOL.Color;

THREEGLMOL.Geometry.prototype.colorAll = function (color) {
   for (var i = 0; i < this.faces.length; i++) {
      this.faces[i].color = color;
   }
};

THREEGLMOL.Matrix4.prototype.isIdentity = function() {
   for (var i = 0; i < 4; i++)
      for (var j = 0; j < 4; j++) 
...
// Reference: A. Bondi, J. Phys. Chem., 1964, 68, 441.
   ...
   this.WIDTH = this.WIDTH * this.aaScale, this.HEIGHT = this.HEIGHT * this.aaScale;
   
   
   this.ASPECT = this.WIDTH / this.HEIGHT;
   this.NEAR = 1, FAR = 800;
   this.CAMERA_Z = -150;
   this.renderer = new THREEGLMOL.WebGLRenderer({antialias: true});
   ...
   this.camera = new THREEGLMOL.PerspectiveCamera(20, this.ASPECT, 1, 800); // will be updated anyway
   this.camera.position = new TV3(0, 0, this.CAMERA_Z);
   this.camera.lookAt(new TV3(0, 0, 0));
   this.perspectiveCamera = this.camera;
   this.orthoscopicCamera = new THREEGLMOL.OrthographicCamera();
   ...
   // UI variables
   this.cq = new THREEGLMOL.Quaternion(1, 0, 0, 0);
   this.dq = new THREEGLMOL.Quaternion(1, 0, 0, 0);
...
GLmol.prototype.setupLights = function(scene) {
   var directionalLight =  new THREEGLMOL.DirectionalLight(0xFFFFFF);
   directionalLight.position = new TV3(0.2, 0.2, -1).normalize();
   directionalLight.intensity = 1.2;
   scene.add(directionalLight);
   var ambientLight = new THREEGLMOL.AmbientLight(0x202020);
...
// described in CONECT. But what about 2JYT???
         ...
         if (type == 290 && line.substr(13, 5) == 'SMTRY') {
            ...
            if (protein.symMat[m] == undefined) protein.symMat[m] = new THREEGLMOL.Matrix4().identity();
            ...
            var n = parseInt(line[18]) - 1;
            var m = parseInt(line.substr(21, 2));
            if (protein.biomtMatrices[m] == undefined) protein.biomtMatrices[m] = new THREEGLMOL.Matrix4().identity();
...
GLmol.prototype.drawAtomsAsSphere = function(group, atomlist, defaultRadius, forceDefault, scale) {
   var sphereGeometry = new THREEGLMOL.SphereGeometry(1, this.sphereQuality, this.sphereQuality); // r, seg, ring
   for (var i = 0; i < atomlist.length; i++) {
      var atom = this.atoms[atomlist[i]];
      if (atom == undefined) continue;

      var sphereMaterial = new THREEGLMOL.MeshLambertMaterial({color: atom.color});
      var sphere = new THREEGLMOL.Mesh(sphereGeometry, sphereMaterial);
      group.add(sphere);
...
GLmol.prototype.drawAtomsAsIcosahedron = function(group, atomlist, defaultRadius, forceDefault) {
   ...
   for (var i = 0; i < atomlist.length; i++) {
      ...
      var mat = new THREEGLMOL.MeshLambertMaterial({color: atom.color});
      var sphere = new THREEGLMOL.Mesh(geo, mat);
...
GLmol.prototype.drawBondsAsStick = function(group, atomlist, bondR, atomR, ignoreNonbonded, multipleBonds, scale) {
   var sphereGeometry = new THREEGLMOL.SphereGeometry(1, this.sphereQuality, this.sphereQuality);
...
GLmol.prototype.drawUnitcell = function(group) {
    ...
    var geo = new THREEGLMOL.Geometry();
    for (var i = 0; i < edges.length; i++) {
       geo.vertices.push(new TV3(vertices[edges[i]][0], vertices[edges[i]][1], vertices[edges[i]][2]));
    }
   var lineMaterial = new THREEGLMOL.LineBasicMaterial({linewidth: 1, color: 0xcccccc});
   var line = new THREEGLMOL.Line(geo, lineMaterial);
   line.type = THREEGLMOL.LinePieces;
   group.add(line);
};
...
GLmol.prototype.drawBondsAsLine = function(group, atomlist, lineWidth) {
   ...
   var lineMaterial = new THREEGLMOL.LineBasicMaterial({linewidth: lineWidth});
...
GLmol.prototype.drawSmoothCurve = function(group, _points, width, colors, div) {
  ...
  var lineMaterial = new THREEGLMOL.LineBasicMaterial({linewidth: width});
  lineMaterial.vertexColors = true;
  var line = new THREEGLMOL.Line(geo, lineMaterial);
  line.type = THREEGLMOL.LineStrip;
  group.add(line);
};
...
GLmol.prototype.drawAsCross = function(group, atomlist, delta) {
  ...
  var lineMaterial = new THREEGLMOL.LineBasicMaterial({linewidth: this.lineWidth});
  lineMaterial.vertexColors = true;
  var line = new THREEGLMOL.Line(geo, lineMaterial, THREEGLMOL.LinePieces);
  group.add(line);
};
...
//      var dir = 1, offset = 0;
   ...
   var mat = new THREEGLMOL.MeshLambertMaterial();
   mat.vertexColors = THREEGLMOL.FaceColors;
   var mesh = new THREEGLMOL.Mesh(geo, mat);
...
GLmol.prototype.drawStrip = function(group, p1, p2, colors, div, thickness) {
   ...
   for (var i = 1, lim = p1.length; i < lim; i++) {
      ...
      for (var j = 0; j < 4; j++) {
         var f = new THREEGLMOL.Face4(offset + faces[j][0], offset + faces[j][1], offset + faces[j][2], offset + faces[j][3], undefined, color);
         fs.push(f);
      }
   }
   ...
   vsize += 8;
   fs.push(new THREEGLMOL.Face4(vsize, vsize + 2, vsize + 6, vsize + 4, undefined, fs[0].color));
   fs.push(new THREEGLMOL.Face4(vsize + 1, vsize + 5, vsize + 7, vsize + 3, undefined, fs[fs.length - 3].color));
   geo.computeFaceNormals();
   geo.computeVertexNormals(false);
   var material =  new THREEGLMOL.MeshLambertMaterial();
   material.vertexColors = THREEGLMOL.FaceColors;
   var mesh = new THREEGLMOL.Mesh(geo, material);
   mesh.doubleSided = true;
   group.add(mesh);
};
...
GLmol.prototype.drawThinStrip = function(group, p1, p2, colors, div) {
   ...
   for (var i = 1, lim = p1.length; i < lim; i++) {
      var f = new THREEGLMOL.Face4(2 * i, 2 * i + 1, 2 * i - 1, 2 * i - 2);
...
GLmol.prototype.IcosahedronGeometry = function() {
   if (!this.icosahedron) this.icosahedron = new THREEGLMOL.IcosahedronGeometry(1);
...
GLmol.prototype.drawCylinder = function(group, from, to, radius, color, cap) {
   ...
   if (!this.cylinderGeometry) {
      this.cylinderGeometry = new THREEGLMOL.CylinderGeometry(1, 1, 1, this.cylinderQuality, 1, !cap);
      this.cylinderGeometry.faceUvs = [];
      this.faceVertexUvs = [];
   }
   var cylinderMaterial = new THREEGLMOL.MeshLambertMaterial({color: color.getHex()});
   var cylinder = new THREEGLMOL.Mesh(this.cylinderGeometry, cylinderMaterial);
   cylinder.position = midpoint;
   cylinder.lookAt(from);
   cylinder.updateMatrix();
   cylinder.matrixAutoUpdate = false;
   var m = new THREEGLMOL.Matrix4().makeScale(radius, radius, from.distanceTo(to));
   m.rotateX(Math.PI / 2);
...
GLmol.prototype.drawNucleicAcidLadder = function(group, atomlist) {
   var geo = new THREEGLMOL.Geometry();
   var lineGeo = new THREEGLMOL.Geometry();
   ...
   geo.computeFaceNormals();
   var mat = new THREEGLMOL.MeshLambertMaterial();
   mat.vertexColors = THREEGLMOL.VertexColors;
   var mesh = new THREEGLMOL.Mesh(geo, mat);
   mesh.doubleSided = true;
   group.add(mesh);
};
...
GLmol.prototype.drawNucleicAcidLine = function(group, atomlist) {
   ...
   var mat =  new THREEGLMOL.LineBasicMaterial({linewidth: 1, linejoin: false});
   mat.linewidth = 1.5; mat.vertexColors = true;
   var line = new THREEGLMOL.Line(geo, mat, THREEGLMOL.LinePieces);
   group.add(line);
};
...
GLmol.prototype.drawDottedLines = function(group, points, color) {
    ...
    var mat = new THREEGLMOL.LineBasicMaterial({'color': color.getHex()});
    mat.linewidth = 2;
    var line = new THREEGLMOL.Line(geo, mat, THREEGLMOL.LinePieces);
    group.add(line);
};
...
GLmol.prototype.drawSymmetryMates2 = function(group, asu, matrices) {
   ...
   var cnt = 1;
   this.protein.appliedMatrix = new THREEGLMOL.Matrix4();
   for (var i = 0; i < matrices.length; i++) {
      var mat = matrices[i];
      if (mat == undefined || mat.isIdentity()) continue;
      console.log(mat);
      var symmetryMate = THREEGLMOL.SceneUtils.cloneObject(asu);
...
GLmol.prototype.drawSymmetryMatesWithTranslation2 = function(group, asu, matrices) {
   ...
   for (var i = 0; i < matrices.length; i++) {
      ...
      for (var a = -1; a <=0; a++) {
         for (var b = -1; b <= 0; b++) {
             for (var c = -1; c <= 0; c++) {
                var translationMat = new THREEGLMOL.Matrix4().makeTranslation(
...
GLmol.prototype.initializeScene = function() {
   ...
   this.scene = new THREEGLMOL.Scene();
   this.scene.fog = new THREEGLMOL.Fog(this.bgColor, 100, 200);

   this.modelGroup = new THREEGLMOL.Object3D();
   this.rotationGroup = new THREEGLMOL.Object3D();
   this.rotationGroup.useQuaternion = true;
   this.rotationGroup.quaternion = new THREEGLMOL.Quaternion(1, 0, 0, 0);
...
GLmol.prototype.setSlabAndFog = function() {
   ...
   if (this.camera instanceof THREEGLMOL.PerspectiveCamera) {
...
GLmol.prototype.enableMouse = function() {
   ...
   glDOM.bind('mousemove touchmove', function(ev) { // touchmove
      ...
      if (mode == 3 || (me.mouseButton == 3 && ev.ctrlKey)) { // Slab
         ...
         var qinv = new THREEGLMOL.Quaternion(q.x, q.y, q.z, q.w).inverse().normalize(); 
         ...
         me.dq.z =  rs * dx; 
         me.dq.w =  rs * dy;
         me.rotationGroup.quaternion = new THREEGLMOL.Quaternion(1, 0, 0, 0); 
