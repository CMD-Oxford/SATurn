...
class AnnotationSupplier {
    public function new() {

    }

    public function annotate(sequence : String, name : String, annotationName : String, config : Dynamic, cb : Dynamic->Array<MoleculeAnnotation>->Void){
        var m = new Molecule(sequence);
        m.setMoleculeName(name);

        annotateMolecule(m, annotationName, config, cb);
    }
